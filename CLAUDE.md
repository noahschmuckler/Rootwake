...
# Active phone study: The Village (E1)

On branch `feat/village`, read **VILLAGERS.md** (the brief and pass plan for the hobbit villagers), **EXPANSION.md** (the agreed, not-yet-built plan for village growth — quickening, construction — and Hulda's own ascent: other lairs and ruins, the danger gradient, the karst as a launching station with a spring, a fisheye view and shrine-teleport, and cultivation tiers gated by world-state) and **VILLAGE_HANDOFF.md** (V0, V1 and W1 as built: eight hobbits in six houses on a day's rhythm, gathering from the land into visible stores and eating from them three times a day, praying at the stone when supplied; Hulda gathers for them by standing in rings and spends their prayer on forest spirits; the Dark Young raid the stores by night and she fights them with a strike, a thorn burst and a root bind; the land beyond the meadow is chunked and procedural, the karst free flow stands in it 330 m north as a feature, and a pinch opens the overhead view and the explored map; the dark forest 400 m off drains her, the mother of goats waits at its heart, and kills level her with a choice each level; two more villages of the same folk under other names stand far out on the karst's safe side, each with its own wolves, and the one she stands in is hers). Live: https://noahschmuckler.github.io/Rootwake/village/ . Earlier studies, each its own route and handoff: the karst free flow (`KARST_FLOW_HANDOFF.md`, `/karst-flow/`), the clearing (`FLOW_HANDOFF.md`, `/flow/`, with the Hulda character work in `CHARACTER_HANDOFF.md`), the karst (`KARST_HANDOFF.md`, `/karst/`), the watershed (`WATERSHED_HANDOFF.md`, `/watershed/`), the ravine (`RAVINE_HANDOFF.md`, `/ravine/`) and the first study (`ROOT_STUDY.md`, `/root-study/`). Preserve the shared player and mobility; each study only adds its own entry. Foliage is drawn as crossed standees (`src/sprites.ts`); roots as opaque tapered tubes (`src/flora.ts`); every figure wears the X Bot skeleton (`src/huldaCharacter.ts`) and is driven by the Mixamo clips in `public/models/clips`.
