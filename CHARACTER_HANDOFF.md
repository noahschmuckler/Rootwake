...
## Hulda on the X Bot's skeleton
Noah asked whether Hulda could have the X Bot's proportions and rigging so the same animations drive her. She does now: `src/huldaSkeleton.json` is the X Bot's bind pose (65 `mixamorig` bones, names, hierarchy, T-pose, centimetres, facing +Z), extracted from `public/models/xbot.fbx` through its inverse bind matrices; `createHulda` builds real `THREE.Bone`s from it under a rig group scaled to the 0.72 m capsule and turned to face −Z, and dresses them: an ellipsoid along every bone toward its child (limbs, fingers, feet in bark, torso along the spine, chest, neck), her head at the rig's size with the face at +Z of the head bone, the vine locks behind, the leaf skirt on the hips, leaf blades on the chest, forearms and shins.
- **Clips drive her directly.** `hulda.setClips(clips)` (through `presentation.setClips`) puts Mixamo clips on her bones by name with the same gait driver as a loaded body (`installGait`: idle / walk / run weighted by the blend, paced by speed, root travel pinned). The clearing loads the three gait clips from `public/models/clips` onto her by default; `?body=xbot` shows the X Bot body with the same clips; `?clips=` (empty) leaves her to the procedural gait.
- **The procedural gait poses the same bones**, in the rig's frame: a rest pose that hangs the T-pose arms (baked into the arm bones' rest so nested swings compose as the old Euler rig did), and `pose(bone, R)` = rest · conj(restWorld) · R · restWorld, so a positive swing takes a hanging limb forward whatever the bone's bind orientation. The lean, knee and elbow signs are as corrected before.
- **The fold** (crouch, tuck, wrap) is applied on top of whichever posed the bones, a slerp toward fixed fold rotations, so the transformations into wood work over a clip too.
- Tests: bone names and parents match the JSON, height and feet, arms hang, faces −Z, a Mixamo-named clip moves her leg and leaves unnamed bones at bind, the fold over a clip, back to bind, the procedural gait on the same bones.
- Jump, the strafes and the turns in `public/models/clips` are still not driven.
...
- **The pack.** Noah supplied Mixamo's Female Basic Locomotion Pack: `public/models/xbot.fbx` is the X Bot (1.7 MB, 147k vertices in two skinned meshes, `mixamorigHips` root) and `public/models/clips/` holds idle, walking, running, jump, the strafes and the turns exported without skin. The build finds the model and the three gait clips (`__HULDA_CLIPS__`), the loader names each clip after its file (inside they are all "mixamo.com"), and the deploy copies `dist/models` beside the page. A `?model=` trial loads without the built-in clips unless `?clips=` names some. Jump, the strafes and the turns are not driven yet: hops, strafing and turning in place are the next things to wire to them.
Not done: retargeting a foreign skeleton to the crouch-and-tuck fold (a skinned model only shrinks and settles to idle before the bark wraps it); the X Bot is a placeholder body for the locomotion, not Hulda's look.
