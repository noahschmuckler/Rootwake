...
- (2026-09-24, M1 decisions) The thought bubble works as it is. The pinch is continuous, shoulder to overhead, the map past the end (i.b). The karst is merged now as a feature of the chunked land, both entries' modes merged (ii.c). The lair 400 m off, the forest 150 m across (iii.a). A level counts kills only (iv.a) and grants a choice each level between vigor, strike and sap (iv.c). W1 stands: a drop-off ring per store; match-3 not in the miracles.
...
- The wider world (M1), four choices before building (the builder's recommendation first in each):
  1. *The pinch and the overhead view.* Scope: how the camera zooms out, what the tactical view is, and whether a fight can be run from it (the stick stays camera-relative, so in the top-down view forward is up the screen; the strike keeps turning her to the nearest). Options: (a) three fixed steps, shoulder (3 m), high (12 m, 45° down), overhead (35 m, near top-down), and a fourth pinch opens the map: discrete steps read on a phone and each is a known framing; (b) a continuous pinch from 3 m to 40 m with the map past the end; (c) two steps only, shoulder and overhead.
  2. *The karst, for now.* Scope: how the karst joins the world in this pass. Options: (a) a crossing: reaching the karst's region on foot (about 300 m north) saves and opens the karst entry, and a trail at the karst floor's edge comes back: connected by a load, honestly; (b) a marker only, the land continuing but the karst out of reach until merged; (c) merge now: the pillar as a feature in the chunked land with both entries' modes merged, the largest lift, a pass or two on its own.
