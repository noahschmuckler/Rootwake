...
## What is built
- **Third person is native.** The shared player in its third view, over the shoulder; the same drag orbits the camera in every form she takes (since the village's V0.2 every form's camera is the shared third person's rule: behind her by yaw, a fixed lift, the look tilted by pitch). A single small button switches to first person and back. The only other button is help.
...
- **Roots.** The ground goes glassy; she is a small bulge riding the network. The stick's direction along the root drives her; pushing back reverses; at a tree she takes whichever root is aligned with the stick; where a root tapers to nothing she stops. Double tap the stick to rise and reform on the ground above; on the ground, double tap the stick to sink from wherever she stands to the nearest root (since the village's V0.2 the double tap lives on the stick only; the screen has none). The network joins every one of the fifteen trees (tested) and the outer trees send single roots outward that taper visibly to dead ends.
