...
try{
 ...
 await v(page,()=>{const k=window.__village;const st=k.stations.find(s=>s.id==='deliver-berries');k.standAt(st.x,st.z,0);});await page.waitForTimeout(2500);
 const handed=await v(page,()=>({stack:window.__village.stack,berries:window.__village.stores.berries}));assert.ok(!handed.stack||handed.stack.n<=1,`handed over (${JSON.stringify(handed)})`);assert.ok(handed.berries>=7,`the baskets hold it (${handed.berries})`);
 await page.screenshot({path:out+'/02e-stack.png'});
 const prayed=await v(page,()=>{const k=window.__village;k.stores.milk=8;k.stores.grain=12;k.advance(200-k.tick);for(let i=0;i<20;i++){k.advance(10);if(k.praying().length>0)break;}return {praying:k.praying(),prayer:k.prayer};});assert.ok(prayed.praying.length>0,`supplied, someone prays (${JSON.stringify(prayed)})`);
