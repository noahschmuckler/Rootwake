...
try{
 ...
 await doubleTapStick(page);await waitMode(page,'sink',5000);await waitMode(page,'root',10000);await doubleTapStick(page);await waitMode(page,'ground',10000);
 ...
 assert.deepEqual(errors,[]);report.push({passed:true,character:'Hulda on the X Bot skeleton, '+builtIn.bones+' bones, clips '+Object.values(builtIn.roles).join(', '),riggedModel:'sample-rig.gltf: Idle / Walking / Running by name, run clip at speed, idle at rest',thirdPerson:true,trunk:true,crown:true,hop:true,roots:'down from the trunk and by a stick double tap; out by another',climb:'up and down',ivy:'grown, kept, descended, reconnected',trail:true,saveReload:true,viewports:4});await writeFile(out+'/results.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
