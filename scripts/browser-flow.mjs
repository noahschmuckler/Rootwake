...
try{
 ...
 // The built-in rigged character (public/models: Mixamo's X Bot with its locomotion clips) loads and takes over before the journey begins.
 await page.waitForFunction(()=>window.__clearing.model.status!=='loading',null,{timeout:120000});const builtIn=await page.evaluate(()=>window.__clearing.model);
 if(builtIn.url){assert.equal(builtIn.status,'ready',builtIn.error);assert.deepEqual(builtIn.roles,{idle:'idle',walk:'walking',run:'running'},JSON.stringify(builtIn.roles));assert.ok(await page.evaluate(()=>!window.__clearing.hulda.group.visible),'the rigged character stands in for the procedural one');await page.waitForTimeout(400);await page.screenshot({path:out+'/01b-rigged-character.png'});}
 ...
 assert.deepEqual(errors,[]);report.push({passed:true,builtInCharacter:builtIn.url?builtIn.url+' with '+Object.values(builtIn.roles).join(', '):'none',riggedModel:'sample-rig.gltf: Idle / Walking / Running by name, run clip at speed, idle at rest',thirdPerson:true,trunk:true,crown:true,hop:true,roots:'down from the trunk and by ground double tap; out by stick double tap',climb:'up and down',ivy:'grown, kept, descended, reconnected',trail:true,saveReload:true,viewports:4});await writeFile(out+'/results.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
