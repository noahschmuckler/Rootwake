...
export function buildRavine(scene: THREE.Scene) {
  ...
  for(let i=0;i<35;i++){const x=rand()*36-19,z=rand()*29-14;if(!onBank(x,z)||Math.hypot(x+9,z-4)<3||Math.hypot(x-10,z+7)<3||Math.hypot(x+9,z-8)<3||Math.abs(z-(5-(x+9)*12/19))<2)continue;tree(x,z,.6+rand()*.5,leaves);}
  ...
  function update(p:Progress,under:number,t:number,eye?:THREE.Vector3){roots.visible=under>.02;earth.opacity=1-under*.88;rock.opacity=1-under*.91;rock.depthWrite=under<.5;earth.depthWrite=under<.5;for(const e of EDGES){const m=edgeMeshes.get(e.id)!;m.opacity=isOpen(e,p)?1:.22;m.emissiveIntensity=isOpen(e,p)?.7:.05;}for(const m of motes){m.mesh.visible=isOpen(m.edge,p);m.mesh.position.copy(m.edge.curve.getPointAt((t*.00007+m.offset)%1)).add(vec(0,-.18,0));}for(const [id,m] of markers){m.visible=id!=='under'&&(!eye||eye.distanceTo(m.position)>1.5);m.scale.setScalar(1+Math.sin(t*.002)*.12);}}
