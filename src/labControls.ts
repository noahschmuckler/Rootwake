...
export function installLabControls(gallery: RustMonsterGallery, player: Player, camera: THREE.PerspectiveCamera, course: MobilityCourse, trialLegs: (on: boolean) => boolean): void {
  ...
  const rootsLink=document.createElement('a');rootsLink.href=location.pathname.includes('/lab/')?'../roots/':'./rootvision.html';rootsLink.textContent='Hulda: beneath the living world ↗';rootsLink.style.cssText='display:block;color:#d0ddad;padding:8px 2px;text-decoration:none';panel.appendChild(rootsLink);
