...
export function installLabControls(gallery: RustMonsterGallery, player: Player, camera: THREE.PerspectiveCamera): void {
  ...
  const frameView = (): void => {
    ...
    camera.fov = bay ? (bay.id === 'surface' ? (camera.aspect < 0.8 ? 118 : 72) : (camera.aspect < 0.8 ? 78 : 60)) : 40;
  ...
  const view = (id: string): void => {
    ...
    } else player.position.set(0, GROUND_Y, 7.2);
  ...
  if (initial && Array.from(views.options).some(o => o.value === initial)) {
    views.value = initial;
    // bootUnder installs the world's ground-height function after populate().
