...
export function installMobilityControls(player: Player): void {
  ...
  player.onMobilityFrame = () => {
    ...
    if (player.targeting) status.textContent = selected ? `${selected.plan.kind.toUpperCase()}  ${(selected.plan.kind === 'drift' ? selected.plan.to.distanceTo(selected.plan.from) : Math.hypot(selected.plan.to.x - selected.plan.from.x, selected.plan.to.z - selected.plan.from.z)).toFixed(1)} m  |  ${selected.plan.kind === 'drift' ? 'depth' : 'height'} ${selected.plan.to.y - selected.plan.from.y >= 0 ? '+' : ''}${(selected.plan.to.y - selected.plan.from.y).toFixed(1)} m\nRelease to move. Return to centre to cancel.` : player.free ? 'AIM: direction + distance select a point in reach.\nLook up or down to aim deeper. Centre cancels.' : 'AIM: direction + distance select a safe landing.\nGreen: walk / gold: jump / blue: drop. Centre cancels.';
    else if (player.free) status.textContent = `DRIFT  ${player.motor.speed.toFixed(1)} m/s\nForward follows your gaze. Hold centre for targets.`;
