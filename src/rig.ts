 ...
 */
function buildFlower(colorIndex: number): { group: THREE.Group; petalMaterial: THREE.MeshStandardMaterial } {
...
export function buildRig(colors: number[]): Rig {
  ...
  for (let i = 0; i < TIP_COUNT; i++) {
    ...
    const { group: flower, petalMaterial } = buildFlower(colors[i]);
  ...
  // Faint cube outline so the "voxel" reads as an object, not five stems in a void.
  const edges = new THREE.LineSegments(
    new THREE.EdgesGeometry(new THREE.BoxGeometry(HALF * 2, HALF * 2, HALF * 2)),
    new THREE.LineBasicMaterial({ color: 0x3a4a3a, transparent: true, opacity: 0.35 })
  );
  root.add(edges);

  return { root, branches, hitTargets };
}
