...
export function rootNetworkWorld(scene: THREE.Scene) {
  ...
  function update(roots: WorldRoot[]) {
    const chunks = new Map<string, WorldRoot[]>();
    for (const r of roots) { const key = r.id.split(':')[1]; const list = chunks.get(key) ?? []; list.push(r); chunks.set(key, list); }
  ...
  return { update, vision(under:number) { visible = under > 0.01; for (const mesh of meshes.values()) mesh.visible = visible; material.emissiveIntensity=0.1+under*0.4; }, get count(){return meshes.size;} };
}
