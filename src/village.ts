...
import type { HobbitState } from './villageModel';
...
let notice = { text: '', until: 0 }, toldEvent = -1;
...
function stations(dt: number): void {
  ...
  if (mode === 'ground') { const held = village.raiders.find(r => r.rooted > 0 && r.held > 0 && r.state !== 'melting' && Math.hypot(r.x - f.x, r.z - f.z) <= HOLD_RANGE); if (held) say = held.state === 'retreating' ? `held in the roots · ${Math.ceil(HOLD_MELT_S - held.held)} s to melt` : 'held in the roots'; }
...
function presentRaiders(dt: number): void {
  ...
  const f = player.feet(); world.setInfants(village.dropped, village.carried && mode !== 'faint' ? new THREE.Vector3(f.x + Math.sin(player.yaw) * -0.25, f.y + 0.85, f.z + Math.cos(player.yaw) * -0.25) : null);
  const near = [...village.snatchers.filter(n => Math.hypot(n.x - f.x, n.z - f.z) < 6).map(n => ({ x: n.x, z: n.z, h: 0.6, text: n.infant ? `something small, with ${hobbitById(n.infant.id).name}` : 'something small' })), ...village.dropped.map(d => ({ x: d.x, z: d.z, h: 0.5, text: hobbitById(d.infant.id).name }))];
...
function frame(now: number) {
  ...
  if (village.events.length && village.events[village.events.length - 1].tick !== toldEvent) { toldEvent = village.events[village.events.length - 1].tick; say(village.events[village.events.length - 1].text, 5); }
