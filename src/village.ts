...
function save() { try { localStorage.setItem(KEY, serializeVillage(village)); localStorage.setItem(OVER_KEY, serializeOverworld(overworld)); } catch { /* Play remains available. */ } }
...
const groundWorld: TraversalWorld = {
  ...
  surfacesAt: (x, z) => [relief(x, z)],
  canOccupy: (p, radius) => p.y >= relief(p.x, p.z) - 0.03 && !HOUSES.some(h => Math.hypot(p.x - h.x, p.z - h.z) < HOUSE_RADIUS + radius) && !treesNear(p.x, p.z, 6).some(t => Math.hypot(p.x - t.x, p.z - t.z) < trunkRadius(t) + radius),
};
...
function frame(now: number) {
  ...
  player.update(now, mode === 'ground' ? [...world.colliders, ...chunks.colliders()] : [], undefined);
