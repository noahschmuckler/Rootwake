...
function frame(now: number) {
  ...
  let wantUnder = 0;
  if (mode === 'ground') { player.applyCamera(camera); const f = player.feet(); if (karst.inside(f.x, f.z)) { if (!village.stack && karst.groundFrame(dt) && mode === 'ground') mode = 'karst'; } else pressInto(dt); }
