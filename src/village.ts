...
import { freshOverworld, parseOverworld, serializeOverworld, explore, isRevealed, knownPlaces, places, dens, danger, addHint, HINT_REACH, HINT_SPREAD, CELL, ZOOM_MIN, ZOOM_MAX, zoomElevation, bearingOf, wrapDeg, type Overworld } from './overworldModel';
...
function save() { try { localStorage.setItem(KEY, serializeVillage(village)); localStorage.setItem(OVER_KEY, serializeOverworld(overworld)); karst?.save(); } catch { /* Play remains available. */ } }
...
if (overworld.seed !== village.seed) overworld = freshOverworld(village.seed);
...
const chunks = createChunks(scene, village.seed, terrain, dens(village.seed)); setTreeProvider((x, z, r) => chunks.treesNear(x, z, r));
// The lair (M1b): placed by the seed (overworldModel), told to the model and set in the scene.
const lairPlace = places(village.seed).find(p => p.id === 'lair')!; setDens(dens(village.seed).map(d => ({ id: d.id, x: d.x, z: d.z, pack: d.pack }))); setLair({ x: lairPlace.x, z: lairPlace.z, radius: lairPlace.radius }); world.setLairAt(lairPlace.x, lairPlace.z);
...
const groundWorld: TraversalWorld = {
  ...
  surfacesAt: (x, z) => (karst.owns(x, z) ? karst.traversal().surfacesAt(x, z) : [relief(x, z)]),
  canOccupy: (p, radius, height) => (karst.owns(p.x, p.z) ? karst.traversal().canOccupy(p, radius, height) : p.y >= relief(p.x, p.z) - 0.03 && !housesOf(village).some(h => Math.hypot(p.x - h.x, p.z - h.z) < HOUSE_RADIUS + radius) && !treesNear(p.x, p.z, 6).some(t => Math.hypot(p.x - t.x, p.z - t.z) < trunkRadius(t) + radius)),
};
...
if (clipUrls.length) import('./huldaModel').then(({ loadHuldaClips }) => loadHuldaClips(clipUrls.map(base))).then(clips => { presentation.setClips(clips); world.setFigureClips(clips); clipStatus = 'ready'; }, e => { clipStatus = 'failed'; clipError = String(e); console.warn('clips', e); });
...
let speed = 1;
function syncShown(): void { for (const s of village.hobbits) { const v = shownFor(s); v.x = s.x; v.z = s.z; v.heading = s.heading; } }
...
function resetVillage(): void { village = freshVillage(village.seed); tickBank = 0; course = null; courseTarget = null; carried = false; syncShown(); pruneShown(); save(); say('the village begins again', 4); }
...
type Mode = 'ground' | 'trunk' | 'crown' | 'hop' | 'sink' | 'grass' | 'root' | 'rise' | 'faint' | 'karst';
...
let blightDrawn = 0;
let notice = { text: '', until: 0 }, toldEvent: VillageEvent | null = null, bannerUntil = 0, heardRumor: Voiced | null = null;
...
const shownMap = new Map<string, Shown>();
function shownFor(s: HobbitState): Shown {
  let v = shownMap.get(s.id); if (v) return v; const h = hobbitById(s.id);
  ...
  v.label.className = 'name'; v.label.textContent = v.name; v.bubble.className = 'bubble'; const meter = document.createElement('i'); meter.className = 'hunger'; v.hunger = document.createElement('b'); meter.append(v.hunger); v.label.append(meter); labels.append(v.label, v.bubble); shownMap.set(s.id, v); return v;
}
function pruneShown(): void { const alive = new Set(village.hobbits.map(s => s.id)); for (const [id, v] of shownMap) if (!alive.has(id)) { v.label.remove(); v.bubble.remove(); shownMap.delete(id); } }
const shown = { get list(): Shown[] { return village.hobbits.map(shownFor); } };
...
function presentHulda(dt: number): void {
  ...
  presentation.update(dt, form, visualPosition, visualRotation, player.motor.speed, heading, mode === 'ground', mode !== 'ground' || player.view === 'third', kv ? kv.present : form);
...
function stations(dt: number): void {
  const f = player.feet(), st = mode === 'ground' ? stationAt(f.x, f.z) ?? siteStation(village, f.x, f.z) : null, id = st?.id ?? null;
  ...
  if (mode === 'ground') { const held = village.raiders.find(r => r.rooted > 0 && r.held > 0 && r.state !== 'melting' && Math.hypot(r.x - f.x, r.z - f.z) <= HOLD_RANGE); if (held && held.id === village.hero.holding) say = `${held.state === 'retreating' ? `holding it · ${Math.ceil(HOLD_MELT_S - held.held)} s to melt` : 'holding it'} · sap ${Math.floor(village.hero.sap)}`; }
  // D3: an infant lying within reach is taken up; carried to its door it is home.
  if (mode === 'ground') { const r = carryInfant(village, f); if (r) save(); }
...
const raiderLabels = new Map<number, { label: HTMLElement; hp: HTMLElement }>(), smallLabels: HTMLElement[] = [];
function presentRaiders(dt: number): void {
  ...
  for (const r of village.raiders) {
    ...
    tmp.set(r.x, relief(r.x, r.z) + 1.9, r.z); const dist = tmp.distanceTo(camera.position); tmp.project(camera); const show = r.state !== 'dead' && r.state !== 'melting' && tmp.z < 1 && dist < 22 && Math.abs(tmp.x) < 1.1;
  ...
  const f = player.feet(); const kept = village.brood.map((b, i) => ({ ...broodSpot(i)!, infant: b.infant })).filter(k => k.x !== undefined); world.setInfants([...village.dropped, ...kept], village.carried && mode !== 'faint' ? new THREE.Vector3(f.x + Math.sin(player.yaw) * -0.25, f.y + 0.85, f.z + Math.cos(player.yaw) * -0.25) : null);
  const near = [...village.snatchers.filter(n => Math.hypot(n.x - f.x, n.z - f.z) < 6).map(n => ({ x: n.x, z: n.z, h: 0.6, text: n.infant ? `something small, with ${hobbitById(n.infant.id).name}` : 'something small' })), ...village.dropped.map(d => ({ x: d.x, z: d.z, h: 0.5, text: hobbitById(d.infant.id).name })), ...kept.map(k => ({ x: k.x, z: k.z, h: 0.5, text: `${hobbitById(k.infant.id).name}, held by the mother` }))];
...
function presentHobbits(dt: number, wall = dt): void {
  ...
  for (const s of village.hobbits) {
    ...
    tmp.set(v.x, relief(v.x, v.z) + f.height + 0.12, v.z); const dist = tmp.distanceTo(camera.position); tmp.project(camera);
...
function drawMap(): void {
  const w = mapEl.width = innerWidth * devicePixelRatio, h = mapEl.height = innerHeight * devicePixelRatio, s = MAP_SCALE * devicePixelRatio, c = mapCtx;
  const sx = (x: number) => w / 2 + (x - mapPan.x) * s, sz = (z: number) => h / 2 + (z - mapPan.z) * s;
  c.fillStyle = '#101a16'; c.fillRect(0, 0, w, h);
  ...
  for (const hnt of overworld.hints) { const a = (hnt.bearing - 90) * Math.PI / 180, r = HINT_REACH * s, sp = HINT_SPREAD * Math.PI / 180; c.fillStyle = '#e0a0f02a'; c.strokeStyle = '#e0a0f0'; c.lineWidth = 1 * devicePixelRatio; c.setLineDash([4 * devicePixelRatio, 4 * devicePixelRatio]); c.beginPath(); c.moveTo(sx(0), sz(0)); c.arc(sx(0), sz(0), r, a - sp, a + sp); c.closePath(); c.fill(); c.stroke(); c.setLineDash([]); c.fillStyle = '#e0a0f0'; c.textAlign = 'center'; const [what, way] = hnt.text.split(/ to the (?=[a-z-]+$)/), tx = sx(0) + Math.cos(a) * r * 0.4, ty = sz(0) + Math.sin(a) * r * 0.4; c.fillText(what, tx, ty); if (way) c.fillText(`to the ${way}`, tx, ty + 14 * devicePixelRatio); }
  for (const p of knownPlaces(overworld)) { const x = sx(p.x), y = sz(p.z); c.strokeStyle = p.id === 'lair' ? '#c070c0' : p.id === 'village' ? STATE_COLOUR[villageState(village).kind] : p.kind === 'den' ? '#c8b8a0' : '#f1edcf'; c.lineWidth = 2 * devicePixelRatio; c.beginPath(); if (p.id === 'village') c.arc(x, y, 7 * devicePixelRatio, 0, Math.PI * 2); else if (p.kind === 'den') { c.fillStyle = '#c8b8a0'; c.arc(x, y, 5 * devicePixelRatio, 0, Math.PI * 2); c.fill(); } else if (p.id === 'karst') { c.moveTo(x, y - 9 * devicePixelRatio); c.lineTo(x + 8 * devicePixelRatio, y + 7 * devicePixelRatio); c.lineTo(x - 8 * devicePixelRatio, y + 7 * devicePixelRatio); c.closePath(); } else { c.moveTo(x - 7 * devicePixelRatio, y - 7 * devicePixelRatio); c.lineTo(x + 7 * devicePixelRatio, y + 7 * devicePixelRatio); c.moveTo(x + 7 * devicePixelRatio, y - 7 * devicePixelRatio); c.lineTo(x - 7 * devicePixelRatio, y + 7 * devicePixelRatio); } c.stroke(); c.fillStyle = c.strokeStyle; c.fillText(p.name, x, y + 24 * devicePixelRatio); }
  ...
  if (village.blight > 0 && knownPlaces(overworld).some(p => p.id === 'lair')) { c.fillStyle = '#3a104060'; c.strokeStyle = '#a050b0'; c.lineWidth = 1.5 * devicePixelRatio; c.beginPath(); c.arc(sx(lairPlace.x), sz(lairPlace.z), village.blight * s, 0, Math.PI * 2); c.fill(); c.stroke(); }
...
function drawCompass(): void {
  ...
  const marks: { x: number; z: number; colour: string; faint?: boolean }[] = knownPlaces(overworld).map(p => ({ x: p.x, z: p.z, colour: p.id === 'lair' ? '#e0a0f0' : p.id === 'village' ? STATE_COLOUR[villageState(village).kind] : p.kind === 'den' ? '#c8b8a0' : '#d8f07a' })); if (courseTarget) marks.push({ ...courseTarget, colour: '#f0d060' });
  // G2: a hint's mark stands where the fan's middle would be, faint, with no distance: they said which way, not how far.
  for (const hnt of overworld.hints) { const a = hnt.bearing * Math.PI / 180; marks.push({ x: Math.sin(a) * HINT_REACH * 0.6, z: -Math.cos(a) * HINT_REACH * 0.6, colour: '#e0a0f088', faint: true }); }
...
const VILLAGE_INFO_M = 75, infoEl = el('village'); let infoClock = 0;
function villageInfo(dt: number): void {
  const f = player.feet(), near = Math.hypot(f.x - GREEN.x, f.z - GREEN.z) <= VILLAGE_INFO_M && !mapOpen; infoEl.hidden = !near; if (!near) return;
  infoClock += dt; if (infoClock < 0.5 && infoEl.textContent) return; infoClock = 0;
  ...
  const rows = [
    `<b style="color:${STATE_COLOUR[vs.kind]}">${vs.kind}</b>${vs.needs.length ? ` · needs: ${vs.needs.join(', ')}` : ''}`,
  ...
  infoEl.innerHTML = rows.map(r => `<div>${r}</div>`).join('');
}
...
function frame(now: number) {
  ...
  if (village.blight !== blightDrawn) { blightDrawn = village.blight; const b = { blight: village.blight }; chunks.setBlight((x, z) => isBlighted(b, x, z)); network.setBlocked((x, z) => isBlighted(b, x, z)); networkScene.reset(); networkScene.update(network.dynamic, network.closed); if (mapOpen) drawMap(); }
  if (network.update(player.feet().x, player.feet().z)) networkScene.update(network.dynamic, network.closed);
  ...
  { const vs = village.voiced, from = heardRumor ? vs.indexOf(heardRumor) + 1 : 0; let added = false; for (let i = from; i < vs.length; i++) { const r = vs[i]; if (r.bearing !== undefined && r.about && addHint(overworld, { about: r.about === 'blight' ? 'lair' : r.about, bearing: r.bearing, text: r.text })) added = true; } if (vs.length) heardRumor = vs[vs.length - 1]; if (added) { save(); if (mapOpen) drawMap(); } }
  { const evs = village.events, from = toldEvent ? evs.indexOf(toldEvent) + 1 : 0; for (let i = from; i < evs.length; i++) { const e = evs[i]; if (e.banner) { bannerEl.textContent = e.text; bannerEl.hidden = false; bannerUntil = time + BANNER_S * 1000; } else say(e.text, 5); } if (evs.length) toldEvent = evs[evs.length - 1]; if (!bannerEl.hidden && time > bannerUntil) bannerEl.hidden = true; }
  const c = clockOf(village.tick); el('clock').textContent = `Day ${c.day} · ${String(c.hour).padStart(2, '0')}:${String(c.minute).padStart(2, '0')}`;
  exploreClock += dt; if (exploreClock > 0.5) { exploreClock = 0; const f = player.feet(); if (explore(overworld, f.x, f.z) > 0 && mapOpen) drawMap(); }
  saveClock += dt; if (saveClock > 5) { saveClock = 0; save(); }
  drawCompass(); renderer.render(scene, camera);
}
