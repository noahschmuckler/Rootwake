...
import { freshOverworld, parseOverworld, serializeOverworld, explore, isRevealed, knownPlaces, places, CELL, ZOOM_MIN, ZOOM_MAX, zoomElevation, bearingOf, wrapDeg, type Overworld } from './overworldModel';
...
const groundWorld: TraversalWorld = {
  ...
  canOccupy: (p, radius, height) => (karst.owns(p.x, p.z) ? karst.traversal().canOccupy(p, radius, height) : p.y >= relief(p.x, p.z) - 0.03 && !HOUSES.some(h => Math.hypot(p.x - h.x, p.z - h.z) < HOUSE_RADIUS + radius) && !treesNear(p.x, p.z, 6).some(t => Math.hypot(p.x - t.x, p.z - t.z) < trunkRadius(t) + radius)),
};
...
let blightDrawn = 0;
let notice = { text: '', until: 0 }, toldEvent: VillageEvent | null = null, bannerUntil = 0;
...
const VILLAGE_INFO_M = 75, infoEl = el('village'); let infoClock = 0, eatenAtDawn = 0, eatenLastNight = 0, infoDay = -1;
function villageInfo(dt: number): void {
  ...
  const c = clockOf(village.tick); if (c.day !== infoDay) { if (infoDay >= 0) eatenLastNight = village.eaten - eatenAtDawn; eatenAtDawn = village.eaten; infoDay = c.day; }
  ...
  const rows = [
    ...
    `last night the Dark Young ate ${eatenLastNight} · melted ${village.melted}`,
...
function frame(now: number) {
  ...
  { const f = player.feet(), depth = forestDepth(f.x, f.z); if (depth > 0) { colour.lerp(forestSky, 0.35 + depth * 0.6); scene.background = colour; scene.fog = new THREE.FogExp2(colour, 0.011 + depth * 0.03); hemi.intensity *= 1 - depth * 0.6; sun.intensity *= 1 - depth * 0.7; } }
  hemi.intensity = 0.5 + 1.9 * light; sun.intensity = 2.3 * light; world.updateLand(village); world.update(light, time, under); chunks.setUnder(under, player.feet().x, player.feet().z); networkScene.vision(under);
