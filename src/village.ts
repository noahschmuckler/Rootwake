...
import { freshOverworld, parseOverworld, serializeOverworld, explore, isRevealed, knownPlaces, places, CELL, ZOOM_MIN, ZOOM_MAX, zoomElevation, type Overworld } from './overworldModel';
...
const chunks = createChunks(scene, village.seed); setTreeProvider((x, z, r) => chunks.treesNear(x, z, r));
...
karst = createKarstFeature(scene, player, camera, KARST_AT, { orbit: (t, back, up) => orbitCamera(t, back, up), want: () => want(), ground: () => groundWorld, locked: lockedWorld });
...
const standable = (x: number, z: number): boolean => grassCan(x, z) && nearestTree(x, z).distance >= trunkRadius(nearestTree(x, z).tree) + 0.3;
...
canvasEl.addEventListener('pointerdown', e => { pinchPointers.set(e.pointerId, { x: e.clientX, y: e.clientY }); if (pinchPointers.size === 2) { pinchFrom = pinchDistance(); pinchZoom = zoom; } }, true);
...
function frame(now: number) {
  ...
  let wantUnder = 0;
  if (mode === 'ground') { player.applyCamera(camera); const f = player.feet(); if (karst.inside(f.x, f.z)) { if (!village.stack && karst.groundFrame(dt)) mode = 'karst'; } else pressInto(dt); }
  else if (mode === 'karst') { karst.update(dt, time, { held: stickHeld, x: stickX(), y: stickY() }); if (karst.mode === 'ground') mode = 'ground'; }
  ...
  if (mode !== 'karst') karst.update(0, time, { held: false, x: 0, y: 0 }); const atm = karst.atmosphere(); if (atm) { colour.copy(atm.colour); scene.background = colour; scene.fog = new THREE.FogExp2(colour, atm.fog); }
  ...
  overheadCamera(); renderer.render(scene, camera);
}
