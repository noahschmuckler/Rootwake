...
function drawMap(): void {
  ...
  for (const key of overworld.revealed) { const [cx, cz] = key.split(',').map(Number); const x = sx(cx * CELL), y = sz(cz * CELL); if (x > -cs && x < w + cs && y > -cs && y < h + cs) { const mx = (cx + 0.5) * CELL, mz = (cz + 0.5) * CELL; c.fillStyle = Math.hypot(mx, mz) < 46 ? '#5b7a48' : '#3e5a3a'; c.fillRect(x, y, cs + 1, cs + 1); } }
