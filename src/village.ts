...
import { freshOverworld, parseOverworld, serializeOverworld, explore, isRevealed, knownPlaces, places, addHint, HINT_REACH, HINT_SPREAD, CELL, ZOOM_MIN, ZOOM_MAX, zoomElevation, bearingOf, wrapDeg, type Overworld } from './overworldModel';
...
function presentRaiders(dt: number): void {
  ...
  for (const r of village.raiders) {
    seen.add(r.id); world.setRaider(r.id, r.x, r.z, r.heading, time, r.state === 'coming' || r.state === 'hunting' || r.state === 'leaving' || r.state === 'retreating', r.hurt, r.rooted, r.state === 'dead' || r.state === 'melting' ? r.gone : 0, r.state === 'retreating' || r.state === 'leaving');
...
function drawMap(): void {
  ...
  for (const p of knownPlaces(overworld)) { const x = sx(p.x), y = sz(p.z); c.strokeStyle = p.id === 'lair' ? '#c070c0' : p.id === 'village' ? STATE_COLOUR[villageState(village).kind] : '#f1edcf'; c.lineWidth = 2 * devicePixelRatio; c.beginPath(); if (p.id === 'village') c.arc(x, y, 7 * devicePixelRatio, 0, Math.PI * 2); else if (p.id === 'karst') { c.moveTo(x, y - 9 * devicePixelRatio); c.lineTo(x + 8 * devicePixelRatio, y + 7 * devicePixelRatio); c.lineTo(x - 8 * devicePixelRatio, y + 7 * devicePixelRatio); c.closePath(); } else { c.moveTo(x - 7 * devicePixelRatio, y - 7 * devicePixelRatio); c.lineTo(x + 7 * devicePixelRatio, y + 7 * devicePixelRatio); c.moveTo(x + 7 * devicePixelRatio, y - 7 * devicePixelRatio); c.lineTo(x - 7 * devicePixelRatio, y + 7 * devicePixelRatio); } c.stroke(); c.fillStyle = c.strokeStyle; c.fillText(p.name, x, y + 24 * devicePixelRatio); }
...
function drawCompass(): void {
  ...
  const marks: { x: number; z: number; colour: string; faint?: boolean }[] = knownPlaces(overworld).map(p => ({ x: p.x, z: p.z, colour: p.id === 'lair' ? '#e0a0f0' : p.id === 'village' ? STATE_COLOUR[villageState(village).kind] : '#d8f07a' })); if (courseTarget) marks.push({ ...courseTarget, colour: '#f0d060' });
