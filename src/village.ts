...
function save() { try { for (const c of ctxs) localStorage.setItem(c.key, serializeVillage(c.v)); localStorage.setItem(OVER_KEY, serializeOverworld(overworld)); karst?.save(); } catch { /* Play remains available. */ } }
...
interface Ctx { i: number; id: string; name: string; short: string; folk: number; v: Village; world: VillageWorld; ox: number; oz: number; key: string; shownMap: Map<string, Shown>; shownSpirits: { x: number; z: number; heading: number }[]; raiderLabels: Map<number, { label: HTMLElement; hp: HTMLElement }>; smallLabels: HTMLElement[]; toldEvent: VillageEvent | null; heardRumor: Voiced | null; near: boolean }
...
const groundWorld: TraversalWorld = {
  ...
  canOccupy: (p, radius, height) => (karst.owns(p.x, p.z) ? karst.traversal().canOccupy(p, radius, height) : p.y >= relief(p.x, p.z) - 0.03 && !houseNear(p.x, p.z, radius) && !treesNear(p.x, p.z, 6).some(t => Math.hypot(p.x - t.x, p.z - t.z) < trunkRadius(t) + radius)),
};
...
if (clipUrls.length) import('./huldaModel').then(({ loadHuldaClips }) => loadHuldaClips(clipUrls.map(base))).then(clips => { presentation.setClips(clips); for (const c of ctxs) c.world.setFigureClips(clips); clipStatus = 'ready'; }, e => { clipStatus = 'failed'; clipError = String(e); console.warn('clips', e); });
...
function fight(simDt: number, dt: number): void {
  ...
  for (const c of ctxs) stepRaiders(c.v, simDt, c === ctx ? her : null, c === ctx);
...
/** Every fresh incident on the land, in the scene's frame. */
const allIncidents = (): { x: number; z: number; text: string; village: string }[] => ctxs.flatMap(c => incidents(c.v).map(i => ({ x: i.x + c.ox, z: i.z + c.oz, text: i.text, village: c.id })));
...
function presentHobbits(c: Ctx, dt: number, wall = dt): void {
  ...
  for (const s of village.hobbits) {
    ...
    const marked = incidents(village).some(i => i.who === s.id), show = !s.inside && tmp.z < 1 && dist < (marked ? 60 : 16) && Math.abs(tmp.x) < 1.1;
...
function drawCompass(): void {
  ...
  const marks: { x: number; z: number; colour: string; faint?: boolean }[] = knownPlaces(overworld).map(p => ({ x: p.x, z: p.z, colour: p.id === 'lair' ? '#e0a0f0' : p.kind === 'village' ? villageColour(p.id) : p.kind === 'den' ? '#c8b8a0' : '#d8f07a' })); if (courseTarget) marks.push({ ...courseTarget, colour: '#f0d060' });
...
function villageInfo(dt: number): void {
  const f = player.feet(), near = Math.hypot(f.x - ctx.ox - GREEN.x, f.z - ctx.oz - GREEN.z) <= VILLAGE_INFO_M && !mapOpen; infoEl.hidden = !near; if (!near) return;
...
function frame(now: number) {
  ...
  overheadCamera();
  fight(Math.min(0.25, wall) * speed, Math.min(0.25, wall)); stations(Math.min(0.25, wall)); presentHulda(dt); for (const c of ctxs) if (c.near) { presentHobbits(c, dt, wall); presentSpirits(c, dt, wall); presentRaiders(c, dt); presentIncidents(c); } presentCourse(); villageInfo(dt);
  ...
  hemi.intensity = 0.5 + 1.9 * light; sun.intensity = 2.3 * light; for (const c of ctxs) { if (c.near) c.world.updateLand(c.v); c.world.setHouses(c.v); c.world.setSite(c.v); c.world.update(light, time, under); } setLayouts(ctxs.map(c => ({ x: c.ox, z: c.oz, huts: c.v.huts }))); chunks.setUnder(under, player.feet().x, player.feet().z); networkScene.vision(under);
  ...
  // G4: every village's talk goes to the map, its fan from that village's green; the village she is in tells her everything, the others only what matters (their banners), named.
  for (const c of ctxs) { const vs = c.v.voiced, from = c.heardRumor ? vs.indexOf(c.heardRumor) + 1 : 0; let added = false; for (let i = from; i < vs.length; i++) { const r = vs[i]; if (r.bearing !== undefined && r.about && addHint(overworld, { about: r.about === 'blight' ? 'lair' : r.about, bearing: r.bearing, text: r.text, ...(c.i ? { from: { x: c.ox, z: c.oz } } : {}) })) added = true; } if (vs.length) c.heardRumor = vs[vs.length - 1]; if (added) { save(); if (mapOpen) drawMap(); } }
  for (const c of ctxs) { const evs = c.v.events, from = c.toldEvent ? evs.indexOf(c.toldEvent) + 1 : 0; for (let i = from; i < evs.length; i++) { const e = evs[i]; if (e.banner) { bannerEl.textContent = c === ctx ? e.text : `${c.short}: ${e.text}`; bannerEl.hidden = false; bannerUntil = time + BANNER_S * 1000; } else if (c === ctx) say(e.text, 5); } if (evs.length) c.toldEvent = evs[evs.length - 1]; }
